...
abstract class FindDeclaredProcsTask : DefaultTask() {
  ...
  private fun procs(artifacts: ArtifactCollection?, classLoader: ClassLoader?): List<AnnotationProcessor> {
    ...
    return artifacts.mapNotNull { artifact ->
      ...
      procs.mapNotNull { procName ->
        inMemoryCache.procs[procName] ?: procFor(artifact, procName, classLoader!!).also { proc ->
          proc?.let { inMemoryCache.procs.putIfAbsent(procName, it) }
        }
      }
    }
  }
  ...
  private fun procFor(
    ...
    return try {
      ...
      logger.quiet("Could not load $procName from class loader")
      null
    }
  }
