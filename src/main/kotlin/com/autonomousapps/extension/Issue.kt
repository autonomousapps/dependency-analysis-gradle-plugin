...
open class Issue @Inject constructor(
  ...
  /**
   * All elements matching the provided patternw ill be filtered out of the final advice. For example:
   * ```
  ...
  fun excludeRegex(@Language("RegExp") vararg patterns: String) {
    excludes.addAll(patterns.map { pattern -> Exclusion.PatternMatch(Regex(pattern, MULTILINE)) })
  }
