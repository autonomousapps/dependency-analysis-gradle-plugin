...
//
  @Nested inner class SingleVariant {
    @Test fun `no advice for correct declaration`() {
      ...
      val usages = usage(Bucket.IMPL, AndroidSourceKind.main("debug")).intoSet()
    ...
    @Test fun `should be api`() {
      ...
      val usages = usage(bucket, AndroidSourceKind.main("debug")).intoSet()
    ...
    @Test fun `should add runtime dependency if already on runtime classpath`() {
      val identifier = "com.foo:bar"
      val usages = usage(Bucket.IMPL, AndroidSourceKind.main("debug")).intoSet()
      val declarations = emptySet<Declaration>()
      val flattened = newSetMultimap<String, String>().apply {
        put(identifier, "debugRuntimeClasspath")
      }
  ...
  @Test fun `shouldn't add runtime dependency if not already on runtime classpath`() {
    val identifier = "com.foo:bar"
    val usages = usage(Bucket.IMPL, AndroidSourceKind.main("debug")).intoSet()
    val declarations = emptySet<Declaration>()
    ...
    @Test fun `should not remove runtimeOnly declarations`() {
      val identifier = "com.foo:bar"
      val usages = usage(Bucket.NONE, AndroidSourceKind.main("debug")).intoSet()
      val declarations = Declaration(
        identifier = identifier,
  ...
  @Nested inner class MultiVariant {
    ...
    @Test fun `no advice for correct declaration`() {
      val identifier = "com.foo:bar"
      val bucket = Bucket.IMPL
      val usages = setOf(usage(bucket, AndroidSourceKind.main("debug")), usage(bucket, AndroidSourceKind.main("release")))
      val declarations = Declaration(
        identifier = identifier,
        configurationName = "implementation",
        gradleVariantIdentification = emptyGVI
