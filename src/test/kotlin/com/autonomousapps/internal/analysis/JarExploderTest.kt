...
internal class JarExploderTest {
   ...
   * Such classes would crash ASM's `ClassReader`. [JarExploder][JarExploder] must
  ...
  @Test fun `does not fail on multi-release classes targeting a future Java version`(@TempDir dir: File) {
    ...
    val expensiveJarsMap = exploder.newEntries

    assertThat(expensiveJarsMap).hasSize(1)
    val classNames = expensiveJarsMap.entries.first().value.binaryClasses.map { it.className }
