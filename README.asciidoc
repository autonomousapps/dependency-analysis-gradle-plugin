...
3.6.3, 4.0.0-beta04, and 4.1.0-alpha08 (`com.android.library` and `com.android.application` projects
only).
...
following in order to get started. What follows is an example using Gradle's runtime API.
...
tasks.register("postProcess") {
  ...
  inputs.file(dependencyAnalysis.adviceOutputFor("debug"))
  ...
  doLast {
    ...
    val advice = inputs.files.singleFile.readText().fromJsonSet<com.autonomousapps.advice.Advice>()
