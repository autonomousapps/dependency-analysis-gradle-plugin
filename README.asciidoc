...
Every team needs a build engineer, but not all teams can afford to have one on staff. Need help? I am available for
https://github.com/autonomousapps[consulting].
...
image::https://img.shields.io/badge/Revved%20up%20by-Develocity-06A0CE?logo=Gradle&labelColor=02303A[Revved up by Develocity,link="https://community.develocity.cloud/scans?search.rootProjectNames=dependency-analysis-gradle-plugin"]
...
=== `org.jetbrains.kotlin:kotlin-metadata-jvm` and the Gradle runtime

For Kotlin projects (whether Android, JVM, or KMP), this project uses the
`org.jetbrains.kotlin:kotlin-metadata-jvm` artifact to read Kotlin metadata. In order to support versions of Gradle
other than the very latest, we isolate that artifact from the main buildscript classpath because Gradle strictly
requires a version compatible with its runtime. See the table
https://docs.gradle.org/9.7.0-rc-1/userguide/compatibility.html[here] for more information. The upshot is that, if you
see an error like

> "Provided Metadata instance has version ..., while maximum supported version is ... To support newer
versions, update the kotlin-metadata-jvm library."

Then you should add the necessary dependency in your build scripts. Consider use of a convention plugin to manage this
globally.
...
----
dependencies {
  "dependencyAnalysisKotlinMetadata"("org.jetbrains.kotlin:kotlin-metadata-jvm:2.4.0")
}
----
